#!/usr/bin/env python3
"""Recipe disentangling for qwen3.6-35b-a3b: self MFQ foundation radars, one panel per
dataset, overlaying base vs organisms (gentle) vs betley (intense) recipe. Tests whether
profile saturation is driven by the dataset or the fine-tuning recipe."""
import csv, sys
from math import pi
...
def prof(stem):
    p = next((x for x in DATA.glob(f"*/{stem}.csv")), None)
...
axes = axes.flatten()
SERIES = [("base", base, "#34495E", "-"), ("organisms (gentle)", None, "#2E8B57", "--"),
          ("betley (intense)", None, "#C0392B", "-")]
handles = labels = None
for ax, ds in zip(axes, DATASETS):
    series = [("base", base, "#34495E", "-"),
              ("organisms (gentle)", prof(f"qwen3.6-35b-a3b-{ds}_temp01_self"), "#2E8B57", "--"),
              ("betley (intense)", prof(f"qwen3.6-35b-a3b-{ds}-betley_temp01_self"), "#C0392B", "-")]
    ...
    for label, p, color, ls in series:
        ...
        vals = p + [p[0]]
        ax.plot(angles, vals, color=color, linestyle=ls, linewidth=2.2, label=label)
...
fig.suptitle("qwen3.6-35b-a3b — recipe drives saturation (self MFQ profiles, T=0.1)\n"
